...
--7
-- inserting values to weekplan table
...
SELECT *
FROM Exercise E
WHERE E.exercise IN (select exercise_name from user_exercise where password = 'hannah')
...
SELECT R.username, IFNULL(num+num1, 0) as numOfSaved
FROM (SELECT username, password FROM userdatabase) R
LEFT OUTER JOIN
    (SELECT U.password, count(exercise_name_entered) as num
    FROM user_exercise_entered UE, userdatabase U
    WHERE U.password = UE.password) R1
ON R.password = R1.password
LEFT OUTER JOIN
    (SELECT U.password, count(exercise_name) as num1
    FROM user_exercise UE, userdatabase U
    WHERE U.password = UE.password) R2
ON R.password = R2.password
WHERE R.password = 'hannah'
